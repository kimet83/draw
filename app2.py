...
# 파일 경로 상수
GIFT_LIMITS_FILE = resource_path('gift_limits.json')
STATE_LOG_DIR = resource_path('logs')
...
def log_draw_result(filename, entries):
    ...
    log_file = os.path.join(STATE_LOG_DIR, 'draw_log.csv')
    log_entries = []
...
def log_delete_result(entry):
    os.makedirs(STATE_LOG_DIR, exist_ok=True)
    log_file = os.path.join(STATE_LOG_DIR, 'delete_log.csv')
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    log_entry = {
        "삭제시각": timestamp,
        "면허번호": entry['면허번호'],
